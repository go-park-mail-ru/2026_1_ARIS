...
// 	Register(ctx context.Context, dto RegisterServiceDTO) (*models.Profile, error)
// 	ValidateRegisterStepOne(ctx context.Context, login, password1, password2 string) (map[string]string, error)
// 	Login(ctx context.Context, dto LoginServiceRequestDTO) (userAccountID int64, err error)
// }
...
// func (s *authService) Register(ctx context.Context, dto RegisterServiceDTO) (*models.Profile, error) {
// 	dto.Login = strings.ToLower(strings.TrimSpace(dto.Login))

// 	if _, err := s.userAccountRepo.GetByUsername(ctx, dto.Login); err == nil {
// 		return nil, errors.New("пользователь с таким login уже существует")
// 	}

// 	birthdayDate, err := time.Parse("02/01/2006", dto.Birthday)
// 	if err != nil {
...
// 	return s.CreateRealUserProfile(ctx, string(hashedPassword), dto.Login, dto.FirstName, dto.LastName, nil, nil, true, birthdayDate, gender, nil)

// 	// по хорошему, надо возвращать DTO
// 	//return profile, nil
// }

// func (s *authService) Login(ctx context.Context, dto LoginServiceRequestDTO) (int64, error) {
// 	dto.Login = strings.ToLower(strings.TrimSpace(dto.Login))

// 	user, err := s.userAccountRepo.GetByUsername(ctx, dto.Login)
// 	if err != nil {
// 		return 0, errors.New("недействительные учётные данные")
// 	}
