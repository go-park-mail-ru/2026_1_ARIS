...
func TestAuthHandler_Login_InvalidBody(t *testing.T) {
	...
	handler := NewAuthHandler(
		mock_service.NewMockAuthService(ctrl),
		mock_service.NewMockSessionService(ctrl),
		mock_service.NewMockUserService(ctrl),
	)
...
func TestAuthHandler_Login_UserProfileNotFound(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	body, _ := json.Marshal(LoginRequest{Login: "ivan123", Password: "qwerty123"})
	req := httptest.NewRequest("POST", "/api/auth/login", bytes.NewReader(body))
	req = req.WithContext(logger.WithLogger(req.Context(), zap.NewNop()))

	mockAuthSvc.EXPECT().
		Login(gomock.Any(), "ivan123", "qwerty123").
		Return(&models.UserAccount{ID: 33}, nil)

	mockUserSvc.EXPECT().
		GetUserProfileByUserID(gomock.Any(), int64(33)).
...
func TestAuthHandler_Register_UserProfileLookupError(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockAuthSvc.EXPECT().
		Register(gomock.Any(), "Ivan", "Petrov", "ivan123", "qwerty123", "24/02/2005", models.Gender("male")).
		Return(&models.Profile{ID: 9}, nil)

	mockUserSvc.EXPECT().
		GetUserProfileByProfileID(gomock.Any(), int64(9)).
...
func TestAuthHandler_Me_InvalidContextUserID(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	req := httptest.NewRequest("GET", "/api/auth/me", nil)
	req = req.WithContext(context.WithValue(req.Context(), "user_id", "bad"))
	req = req.WithContext(logger.WithLogger(req.Context(), zap.NewNop()))
...
func TestAuthHandler_Me_WithAvatar(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockSessionService := mocks.NewMockSessionService(ctrl)
	mockUserService := mocks.NewMockUserService(ctrl)
	mockMediaService := mocks.NewMockMediaService(ctrl)
	...
	mockUserService.EXPECT().
		GetUserProfileByUserAccountID(gomock.Any(), userID).
		Return(&models.UserProfile{ProfileID: 1, FirstName: "Ivan", LastName: "Petrov"}, nil)

	mockUserService.EXPECT().
		GetProfileByUserAccountID(gomock.Any(), userID).
		Return(&models.Profile{AvatarID: &avatarID}, nil)

	mockMediaService.EXPECT().
		GetAvatarByID(gomock.Any(), &avatarID).
		Return(&models.Media{Link: "avatar.png"}, nil)
	...
	err := json.Unmarshal(w.Body.Bytes(), &resp)
	assert.NoError(t, err)
	assert.Equal(t, "avatar.png", resp.AvatarLink)
}

func TestNewAuthHandler_WithoutMediaService(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	assert.Nil(t, handler.mediaService)
}

func TestValidateRegisterStepOne_ServiceErrors(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	reqBody := `{"login":"valid12","password1":"password1","password2":"password1"}`
	req := httptest.NewRequest(http.MethodPost, "/validate-step1", bytes.NewBufferString(reqBody))
	req.Header.Set("Content-Type", "application/json")
	req = req.WithContext(logger.WithLogger(req.Context(), zap.NewNop()))

	mockAuthSvc.EXPECT().
		ValidateRegisterStepOne(gomock.Any(), "valid12", "password1", "password1").
		Return(map[string]string{"login": "already exists"}, nil)
	...
	assert.NoError(t, err)
	assert.False(t, resp.Ok)
	assert.Equal(t, "already exists", resp.Errors["login"])
}

func TestValidateRegisterStepOne_ServiceError(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
