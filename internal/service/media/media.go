...
type MediaService interface {
	...
	Save(ctx context.Context, name string, size int64, fileReader multipart.File, contentType string, authorID int64) (string, error)
}
...
func (s *mediaService) Save(ctx context.Context, name string, size int64, fileReader multipart.File, contentType string, authorID int64) (string, error) {
	mediaUUID := uuid.New()

	mType, extension, err := GetMimeType(fileReader)

	if !isAllowed(mType, contentType) {
		return "", xerrors.UnsupportedContentType
	}
